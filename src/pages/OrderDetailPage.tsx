...
export function OrderDetailPage() {
  ...
  const [showConfirm, setShowConfirm] = useState(() => {
    if (isMockMode() && searchParams.get('created') === '1') return true
  ...
  return <div className="container-page py-8 sm:py-12">
    ...
    {isMockMode() && order.payment_method === 'jawwal_pay' && ['unpaid','failed'].includes(order.payment_status) && order.status === 'pending' && <section className="checkout-card mt-6"><h2 className="text-lg font-bold">إتمام الدفع لهذا الطلب</h2><p className="my-3 text-sm">طلبكِ محفوظ. استئناف الدفع يستخدم الطلب نفسه.</p><button className="btn-primary" disabled={payment.isPending} onClick={() => payment.mutate(order.order_number)}>{payment.isPending ? 'جارٍ المتابعة…' : isMockMode() ? 'تأكيد الدفع التجريبي' : 'المتابعة إلى الدفع'}</button>{payment.isError && <p className="field-error" role="alert">{getApiErrorMessage(payment.error)}</p>}</section>}
