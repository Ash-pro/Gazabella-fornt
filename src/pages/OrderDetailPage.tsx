...
const date = (value: string) => new Intl.DateTimeFormat('ar-PS-u-nu-latn', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(value))
export function OrderDetailPage() {
  ...
  return <div className="container-page py-8 sm:py-12">
    ...
    {searchParams.get('created') === '1' && order.payment_method === 'cash_on_delivery' && <p className="demo-note mb-5" role="status">✓ تم تأكيد طلبكِ {order.order_number} — الدفع عند الاستلام.</p>}
    ...
    <div className="flex flex-wrap items-center justify-between gap-4"><div><span className="eyebrow">كل التفاصيل في مكان واحد</span><h1 className="mt-2 text-2xl font-bold" dir="ltr"><span className="num">{order.order_number}</span></h1><p className="mt-2 text-xs text-[var(--text-3)]">{date(order.created_at)}</p></div><span className={'status-badge status-' + order.status}>{labels[order.status] || order.status}</span></div>
