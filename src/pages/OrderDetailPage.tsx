  ...
  return <div className="container-page py-8 sm:py-12">
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
      ...
      <aside className="space-y-6">
        <section className="order-summary"><h2 className="mb-5 text-lg font-bold">ملخص الطلب</h2><dl className="space-y-3 text-sm">{[['المنتجات',money(order.subtotal)],['التوصيل',money(order.delivery_fee)],['الخصم','−' + money(order.discount_amount)],['طريقة الدفع',order.payment_method === 'cash_on_delivery' ? 'الدفع عند الاستلام' : 'Jawwal Pay'],['حالة الدفع',payments[order.payment_status] || order.payment_status]].map(([label,value]) => <div key={label} className="flex justify-between gap-4"><dt className="text-[var(--text-2)]">{label}</dt><dd className="font-bold">{value}</dd></div>)}</dl><div className="mt-5 flex justify-between border-t border-[var(--border)] pt-5"><b>الإجمالي</b><b className="text-xl text-[var(--primary)]">{money(order.total)}</b></div>{isMockMode() && <p className="mt-4 text-xs leading-6 text-[var(--text-3)]">هذا طلب تجريبي للعرض. لا توجد مدفوعات أو شحنات حقيقية.</p>}</section>
