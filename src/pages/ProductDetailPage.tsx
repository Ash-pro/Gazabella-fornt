...
function ProductContent({slug}: {slug: string}) {
  ...
  return <div className="container-page product-detail">
    ...
    <div className="detail-layout">
      ...
      <div className="detail-copy"><span className="eyebrow">{product.category.name}</span><h1>{product.name}</h1><p className="detail-description">{product.description || 'اكتشفي تفاصيل هذا المنتج واختاري الحجم المناسب لكِ.'}</p><div className="detail-price"><b>{money(variant?.price || 0)}</b>{variant?.compare_at_price && Number(variant.compare_at_price)>Number(variant.price) && <del>{money(variant.compare_at_price)}</del>}<span className={limit ? 'in-stock' : 'out-stock'}>{limit ? 'متوفر' : 'غير متوفر'}</span></div>
      <fieldset className="detail-variants"><legend>اختاري الحجم أو الدرجة</legend><div>{product.variants.map((v)=><button type="button" key={v.id} disabled={!v.available_quantity || add.isPending} aria-pressed={v.id===variant?.id} onClick={()=>{setVariantId(v.id);setQuantity(1)}} className={v.id===variant?.id?'selected':''}><span>{v.name}</span><b>{money(v.price)}</b></button>)}</div></fieldset>
      <div className="detail-buy"><div className="quantity-control"><button disabled={quantity<=1 || add.isPending} aria-label="تقليل الكمية" onClick={()=>setQuantity(q=>q-1)}>−</button><span>{quantity}</span><button disabled={quantity>=limit || add.isPending} aria-label="زيادة الكمية" onClick={()=>setQuantity(q=>q+1)}>+</button></div>{button}</div>{add.isError && <p className="field-error" role="alert">{getApiErrorMessage(add.error)}</p>}
    ...
    <div className="product-sticky"><div><small>{variant?.name}</small><b>{money(variant?.price || 0)}</b></div>{button}</div>
