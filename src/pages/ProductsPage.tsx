...
export function ProductsPage() {
  ...
  const filters: ProductFilters = { category_slug: sub || (params.getAll('category').length === 1 ? category : undefined), category_slugs: !sub && params.getAll('category').length > 1 ? params.getAll('category') : undefined, stores: params.getAll('store'), search: search || undefined, sort: validSort, min_price: numberParam('min_price'), max_price: numberParam('max_price'), page, per_page: 12 }
  ...
  const active = Boolean(params.has('store') || category || sub || search || validSort || filters.min_price !== undefined || filters.max_price !== undefined || savedOnly || page > 1)
  ...
  const chips = [
    ...
    ...params.getAll('store').map((value) => ({key:'store',value,label:value})),
