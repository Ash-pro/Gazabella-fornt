...
export function MerchantDashboard() {
  ...
  return <div className="ops-page"><div className="container-page">
    ...
    {dataError ? <ErrorState message={getApiErrorMessage(dataError)} onRetry={() => void client.invalidateQueries({queryKey:['merchant-stores']}).then(() => Promise.all([stats.refetch(),products.refetch(),orders.refetch()]))} /> : stores.isLoading || stats.isLoading || products.isLoading || orders.isLoading ? <PageLoader /> : <>
      ...
      {tab === 'inventory' && <div className="ops-inventory">{products.data?.map((p) => <article key={p.id}><div className="size-20 overflow-hidden rounded-lg"><ProductVisual src={p.thumbnail_url} alt={p.name} /></div><div className="flex-1 min-w-0"><p className="text-xs text-[var(--text-3)]">{p.category_name}</p><h2 className="my-1 text-sm font-bold">{p.name}</h2><p className="text-xs"><span className="num">{p.variants_count}</span> أنواع · من <span className="num">{money(p.min_price)}</span></p></div><div className="text-center"><b className={`num ${p.total_stock < 5 ? 'text-[var(--warning)]' : ''}`}>{p.total_stock}</b><small className="block text-[10px]">قطعة</small></div><button className="btn-ghost text-xs" disabled={editProduct.isPending} onClick={() => editProduct.mutate(p.slug)}>تعديل المخزون</button></article>)}</div>}
