...
export function MerchantDashboard() {
  ...
  const stores = useQuery({queryKey:['merchant-stores'],queryFn:gazabellaApi.getMerchantStores,staleTime:Infinity})
  const stats = useQuery({queryKey:['merchant-stats',storeId],queryFn:() => gazabellaApi.getMerchantStats(storeId),staleTime:30_000})
  const products = useQuery({queryKey:['merchant-products',storeId],queryFn:() => gazabellaApi.getMerchantProducts(storeId),staleTime:30_000})
  ...
  const editProduct = useMutation({mutationFn:gazabellaApi.getProduct,onSuccess:(p) => {setEditing(p);setStock({[p.id]:String(p.stock)});setError('')},onError:(e) => setError(getApiErrorMessage(e))})
  const saveStock = (event: React.FormEvent) => {
    event.preventDefault()
    if (!editing || !isMockMode()) return
    try {
      const qty = Number(stock[editing.id])
      if (!Number.isInteger(qty) || qty < 0 || qty > 10000 || stock[editing.id] === '') throw new Error('أدخلي كمية صحيحة من 0 إلى 10000.')
      demoSetStock(editing.id, qty)
  ...
  return <div className="ops-page"><div className="container-page">
    ...
    {dataError ? <ErrorState message={getApiErrorMessage(dataError)} onRetry={() => void client.invalidateQueries({queryKey:['merchant-stores']}).then(() => Promise.all([stats.refetch(),products.refetch(),orders.refetch()]))} /> : stores.isLoading || stats.isLoading || products.isLoading || orders.isLoading ? <PageLoader /> : <>
      {tab === 'overview' && <><div className="ops-stats">{[['قيمة طلبات اليوم',money(stats.data?.today_sales || 0)],['طلبات اليوم',stats.data?.today_orders_count ?? 0],['قيد التجهيز',stats.data?.pending_prep_count ?? 0],['جاهز للاستلام',stats.data?.ready_for_pickup_count ?? 0]].map(([label,value]) => <section key={label}><p>{label}</p><b className="num">{value}</b></section>)}</div><section className="checkout-card mt-6"><h2 className="text-lg font-bold">أولويات اليوم</h2><p className="my-4 text-sm leading-7">راجعي الطلبات قيد التجهيز، ثم حدّثي حالتها عند جاهزيتها. يوجد <span className="num">{stats.data?.low_stock_items_count ?? 0}</span> منتج بمخزون منخفض.</p><div className="flex flex-wrap gap-3"><button className="btn-primary" onClick={() => setTab('orders')}>عرض طلبات التجهيز</button><button className="btn-ghost" onClick={() => setTab('inventory')}>مراجعة المخزون</button></div></section></>}
      {tab === 'orders' && (orders.data?.length ? <div className="grid gap-4 md:grid-cols-2">{orders.data.map((item) => <article className="checkout-card" key={item.id}><div className="flex flex-wrap justify-between gap-3"><b className="text-sm text-[var(--primary)] num" dir="ltr">{item.order_number}</b><span className="text-xs">{prepLabels[item.prep_status]}</span></div><h2 className="mt-4 text-base font-bold">{item.product_name}</h2><p className="my-2 text-sm text-[var(--text-2)]">{item.variant_name} · <span className="num">{item.quantity}</span> قطعة · <span className="num">{money(item.subtotal)}</span></p><p className="mb-5 text-xs text-[var(--text-3)]">{item.customer_name} · {item.customer_area}</p>{['pending','preparing'].includes(item.prep_status) && <button className="btn-primary w-full" disabled={prep.isPending} onClick={() => prep.mutate({id:item.id,status:'ready_for_pickup'})}>جاهز لاستلام المندوب</button>}</article>)}</div> : <EmptyState title="لا توجد طلبات تجهيز" message="ستظهر طلبات هذا المتجر هنا بعد تأكيدها." />)}
      {tab === 'inventory' && <div className="ops-inventory">{products.data?.map((p) => <article key={p.id}><div className="size-20 overflow-hidden rounded-lg"><ProductVisual src={p.image_url} alt={p.name} /></div><div className="flex-1 min-w-0"><p className="text-xs text-[var(--text-3)]">{p.category_name}</p><h2 className="my-1 text-sm font-bold">{p.name}</h2><p className="text-xs">المخزون: <span className="num">{p.total_stock}</span> · <span className="num">{money(p.price)}</span></p></div><div className="text-center"><b className={`num ${p.total_stock < 5 ? 'text-[var(--warning)]' : ''}`}>{p.total_stock}</b><small className="block text-[10px]">قطعة</small></div><button className="btn-ghost text-xs" disabled={editProduct.isPending} onClick={() => editProduct.mutate(p.slug)}>تعديل المخزون</button></article>)}</div>}
    ...
    {editing && <Dialog title={'مخزون ' + editing.name} onClose={() => setEditing(null)}><form onSubmit={saveStock} className="space-y-4 p-5"><p className="demo-note">تُحفظ التغييرات محليًا ضمن Mock Mode.</p><label className="field-label block">الكمية المتاحة<input className="form-field mt-2" type="number" min={0} max={10000} step={1} required value={stock[editing.id] ?? ''} onChange={(e) => setStock({...stock,[editing.id]:e.target.value})} /></label>{error && <p className="field-error" role="alert">{error}</p>}<button className="btn-primary w-full">حفظ المخزون</button></form></Dialog>}
  </div></div>
}
