  ...
  return <div className="container-page py-8 sm:py-12">
    ...
    {!checkout.pendingOrder && <form className="grid gap-8 lg:grid-cols-[1fr_360px] lg:items-start" onSubmit={form.handleSubmit((v) => { if (busy || expired || !selected) return; if (step === 1) { setStep(2); return } setSubmitted(true); create.mutate(v) })}>
      <div className="space-y-6">
        ...
        <label className="checkout-card block"><span className="field-label">ملاحظة للتوصيل (اختياري)</span><textarea className="form-field" maxLength={500} {...form.register('notes')} placeholder="أي تفاصيل تساعدنا على الوصول إليكِ" /></label></> : <section className="checkout-card"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">راجعي طلبكِ</h2><button type="button" className="text-link" onClick={() => setStep(1)}>تعديل العنوان</button></div><div className="my-6 space-y-2 text-sm"><b>{values.full_name}</b><p>{values.city}، {values.area}</p><p>{values.details}</p><p dir="ltr" className="text-right">{values.phone}</p><p>{selected?.name}</p></div><div className="rounded-lg bg-[var(--surface2)] p-4 text-sm leading-7"><Icon name="shield" className="mb-2 size-5 text-[var(--primary)]" />{isMockMode() ? 'الدفع عند الاستلام — ادفعي قيمة الطلب نقدًا عند وصوله. هذا طلب تجريبي للعرض فقط.' : 'ستنتقلين إلى صفحة Jawwal Pay لإتمام الدفع. لا نؤكد الدفع إلا بعد تحديث حالة الطلب من الخادم.'}</div></section>}
      </div>
