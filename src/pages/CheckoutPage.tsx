...
export function CheckoutPage() {
  ...
  const begin = useMutation({ mutationFn: (couponCode?: string) => gazabellaApi.beginCheckout(couponCode), onSuccess: (session) => { checkout.setSession(session); useCartStore.getState().setReservation(session.expires_at || session.reserved_until || null, session.seconds_remaining); form.setValue('delivery_option_id', session.delivery_options.find((d) => d.is_available)?.id || 0) } })
  useEffect(() => {
