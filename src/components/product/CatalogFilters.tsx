...
export function CatalogFilters({categories, onClose}: {categories: Category[]; onClose: () => void}) {
  ...
  const catalog = useQuery({queryKey:['filter-options'], queryFn:() => gazabellaApi.getProducts({per_page:1000})})
  const stores = [...new Set(catalog.data?.data.flatMap((p) => p.store ? [p.store.name] : []) || [])]
  const ceiling = Math.max(1000, ...catalog.data?.data.map((p) => Number(p.max_price)) || [], Number(draft.get('max_price')) || 0, Number(draft.get('min_price')) || 0)
  ...
  function toggle(name: string, value: string) { const next = new URLSearchParams(draft); const selected = next.getAll(name); next.delete(name); (selected.includes(value) ? selected.filter((v) => v !== value) : [...selected,value]).forEach((v) => next.append(name,v)); next.delete('page'); if(name === 'category') next.delete('sub'); setDraft(next) }
  ...
  const content = <div className="catalog-filter-content"><fieldset><legend>نطاق السعر</legend><div className="price-range-labels"><span>{min} ₪</span><span>{max} ₪</span></div><div className="dual-range"><input aria-label="أقل سعر" type="range" min="0" max={ceiling} value={min} onChange={(e) => set('min_price',String(Math.min(Number(e.target.value),max)))} /><input aria-label="أعلى سعر" type="range" min="0" max={ceiling} value={max} onChange={(e) => set('max_price',String(Math.max(Number(e.target.value),min)))} /></div><div className="price-inputs"><label>من ₪<input className="form-field" type="number" min="0" max={max} value={min} onChange={(e) => set('min_price',String(Math.max(0,Math.min(Number(e.target.value),max))))}/></label><label>إلى ₪<input className="form-field" type="number" min={min} max={ceiling} value={max} onChange={(e) => set('max_price',String(Math.max(min,Number(e.target.value))))}/></label></div></fieldset><fieldset><legend>الأقسام</legend>{categories.map((c) => <label className="filter-check" key={c.slug}><input type="checkbox" checked={draft.getAll('category').includes(c.slug)} onChange={() => toggle('category',c.slug)}/>{c.name}</label>)}{!!selectedCategory?.children.length && <label className="field-label mt-4">الفئة الفرعية<select className="form-field" value={draft.get('sub') || ''} onChange={(e) => { const next = new URLSearchParams(draft); if(e.target.value) next.set('sub',e.target.value); else next.delete('sub'); next.delete('page'); setDraft(next) }}><option value="">كل الفئات الفرعية</option>{selectedCategory.children.map((c) => <option key={c.slug} value={c.slug}>{c.name}</option>)}</select></label>}</fieldset><fieldset><legend>المتاجر الشريكة</legend>{catalog.isLoading && <p>جارٍ تحميل المتاجر…</p>}{catalog.isError && <button onClick={() => void catalog.refetch()}>إعادة تحميل المتاجر</button>}{stores.map((name) => <label className="filter-check" key={name}><input type="checkbox" checked={draft.getAll('store').includes(name)} onChange={() => toggle('store',name)}/>{name}</label>)}</fieldset><div className="filter-footer"><button className="btn-primary" onClick={() => {setParams(draft,{state:{preserveScroll:true}});onClose()}}>تطبيق الفلاتر</button><button className="text-link" onClick={reset}>إعادة تعيين</button></div></div>
