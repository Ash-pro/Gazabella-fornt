...
export function ProductCard({ product }: { product: ProductBrief }) {
  ...
  return <article className="catalog-card">
    ...
    <div className="catalog-card__body"><span className="catalog-category">{product.category.name}</span><Link className="catalog-name" to={`/products/${product.slug}`}>{product.name}</Link><span className="product-store">🏪 {product.store?.name || 'متجر شريك'}</span><div className="catalog-price">{product.min_price !== product.max_price && <small>من</small>}<b>{money(product.min_price)}</b>{discount > 0 && <del>{money(previous)}</del>}</div><Link className="catalog-action" to={`/products/${product.slug}`}>{product.is_available ? 'اكتشفي المنتج' : 'عرض التفاصيل'}<Icon name="arrow" className="size-4 rotate-180" /></Link><button className="btn-primary quick-add" disabled={!product.is_available || quick.isPending || add.isPending} onClick={() => { add.reset(); quick.mutate() }}>{quick.isPending || add.isPending ? 'جارٍ الإضافة…' : 'أضيفي للسلة +'}</button>{(quick.error || add.error) && !detail && <p role="alert" className="field-error">{getApiErrorMessage(quick.error || add.error)}</p>}</div>
    {detail && <Dialog title="اختاري تفاصيل منتجكِ" bottom onClose={() => setDetail(null)}><div className="quick-selector"><h3>{product.name}</h3><p className="product-store">{product.store?.name}</p><div className="variant-options">{detail.variants.map((v) => <button key={v.id} disabled={!v.available_quantity || add.isPending} aria-pressed={variantId === v.id} className={variantId === v.id ? 'selected' : ''} onClick={() => setVariantId(v.id)}>{v.name}<b>{money(v.price)}</b>{!v.available_quantity && <small>غير متوفر</small>}</button>)}</div>{add.error && <p role="alert" className="field-error">{getApiErrorMessage(add.error)}</p>}<button className="btn-primary w-full" disabled={!variantId || add.isPending} onClick={() => { const v = detail.variants.find((v) => v.id === variantId); if(v) add.mutate(v) }}>{add.isPending ? 'جارٍ الإضافة…' : 'أضيفي للسلة +'}</button></div></Dialog>}
  </article>
}
